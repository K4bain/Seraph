 ...
 *
 * Generates a narrative briefing from a canvas. Returns a typed Briefing
 * object — nothing is written to the canvas. The analyst reviews, then
 * POSTs to /api/ai/apply-briefing to write it as a memo card.
 */

import { AiNotConfiguredError } from "@/core/ai/client";
import { generateBriefing } from "@/core/ai/tasks/briefing";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await withTimeout(generateBriefing(doc));
