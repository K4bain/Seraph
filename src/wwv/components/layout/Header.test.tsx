...
vi.mock("@/wwv/core/hooks/useIsMobile", () => ({
    useIsMobile: () => false,
}));

// Mock store dependencies
vi.mock("@/wwv/core/state/store", () => ({
    useStore: vi.fn((sel) => {
...
vi.mock("@/wwv/core/data/DataBus", () => ({
    dataBus: { emit: vi.fn() },
}));

vi.mock("@/wwv/core/plugins/PluginManager", () => ({
    pluginManager: { updateTimeRange: vi.fn() },
}));

vi.mock("@/wwv/lib/analytics", () => ({
    trackEvent: vi.fn(),
}));
...
describe("Header signout", () => {
    ...
    it("does not render signout button in demo edition", async () => {
        const mod = await import("@/wwv/core/edition");
    ...
    it("renders signout button on mobile when not in demo", async () => {
        const mobileMod = await import("@/wwv/core/hooks/useIsMobile");
